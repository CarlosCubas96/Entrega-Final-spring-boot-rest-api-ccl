...
public class JwtServiceImpl implements JwtServiceI {
	/**
	 * ############# # CLAIMS # ############# En los tokens JWT, los claims
	 * comúnmente incluyen información como:
	 * 
	 * sub (Subject): Identificador del usuario. exp (Expiration Time): Tiempo de
	 * expiración del token. iat (Issued At): Momento en que se emitió el token. iss
	 * (Issuer): Quién emitió el token. aud (Audience): A quién está destinado el
	 * token.
	 */
	...
	// Verifica si el token es válido comparando el nombre de usuario y comprobando
	// si ha expirado.
	@Override
	...
	// Los claims son declaraciones que contienen información sobre el usuario y
	...
	// Genera el token JWT incluyendo los claims adicionales y los detalles del
	// usuario.
	// Los claims adicionales pueden ser usados para almacenar información adicional
	// sobre el usuario o el token.
